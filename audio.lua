...
-- SUPPORT ALL TERRAIN MATERIALS & SUPPORT MOBILE
...
-- Tombol Copy Script Material di Pojok Kanan Atas Frame
...
ConsoleLog.Text = "> Auto Copier Ready (All Terrain Support)"
...
local function generateServerScriptCode(voxelList)
	...
	table.insert(lines, "-- GENERATED BY @IkyyXD (ALL TERRAIN SUPPORT)")
	table.insert(lines, "-- ======================================================")
...
--------------------------------------------------
-- FUNGSI SCAN AUTOMATIS TERRAIN (ALL TERRAIN SUPPORT)
--------------------------------------------------
local function autoScanTerrain()
	...
	local scanRadius = Vector3.new(32, 24, 32) -- Area scan diperbesar
	...
	for x = 1, size.X do
		for y = 1, size.Y do
			for z = 1, size.Z do
				...
				-- DENGAN INI, SEMUA MATERI TERRAIN DISUPPORT (Tanpa filter Air, Rumput, Batu, Dll)
	...
	setConsoleMessage("Scanned " .. count .. " Voxels (All Materials)!", Color3.fromRGB(100, 255, 200))
	return voxels
end
